...
export class GetGridClustersUseCase {
  ...
  async execute(macroSlug: string): Promise<StructuralAsset> {
    // 1. Hole das MacroCluster selbst (für den Titel/Header der Page)
    const macroCluster = await this.assetRepo.findStructuralBySlug(macroSlug);
    ...
    const clusters = await this.assetRepo.findChildren(macroCluster.id);
