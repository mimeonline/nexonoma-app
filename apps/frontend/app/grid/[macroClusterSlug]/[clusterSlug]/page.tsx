...
function filterContents(
  contents: ContentWithSegment[],
  activeSegment: "all" | string,
...
export default function ClusterDetailPage({ params }: PageProps) {
  const { macroClusterSlug, clusterSlug } = use(params);
  ...
  useEffect(() => {
    ...
    fetchGrid()
      .then((response) => {
        const mc = response.data?.macroClusters?.find((m) => m.slug === macroClusterSlug) ?? null;
        const cl = mc?.clusters?.find((c) => c.slug === clusterSlug) ?? null;
        setMacroCluster(mc);
        setCluster(cl ?? null);
      })
      .catch((err) => setError(err instanceof Error ? err.message : "Unbekannter Fehler"))
      .finally(() => setLoading(false));
  }, [macroClusterSlug, clusterSlug]);

  const contents = useMemo(() => (cluster ? flattenContents(cluster) : []), [cluster]);
  const filtered = useMemo(
    () => filterContents(contents, activeSegment, activeType),
    [contents, activeSegment, activeType],
  );

  if (!loading && !error && (!macroCluster || !cluster)) {
    return notFound();
  }

  const segments: Segment[] = cluster?.segments ?? [];
  const segmentsToRender = activeSegment === "all"
    ? segments
    : segments.filter((s) => s.slug === activeSegment);

  return (
    <div className="min-h-screen bg-linear-to-b from-[#0B1220] to-[#111827] text-white">
      <main className="mx-auto flex max-w-6xl flex-col gap-6 px-6 pb-12 pt-10 sm:pt-16 sm:pb-16">
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div className="space-y-2">
              <nav className="mb-2 flex items-center gap-2 text-sm text-slate-400">
                ...
                <span>/</span>
                <Link href={`/grid/${macroCluster?.slug ?? macroClusterSlug}`} className="hover:text-slate-200">
                ...
                {viewToggle.map((toggle) => (
                  <button
                    ...
                    className={`flex items-center gap-2 rounded-md px-3 py-1 transition ${
                      viewMode === toggle.key
                        ? "bg-nexo-aqua/20 text-nexo-aqua border border-nexo-aqua/40 shadow"
          ...
          <div className="mt-6 flex items-center gap-4 overflow-x-auto border-b border-white/5 pb-2 text-sm text-slate-300">
            <button
              ...
              className={`pb-2 transition ${
                activeSegment === "all"
                  ? "border-b-2 border-nexo-aqua text-white"
                  : "hover:text-white/80"
              }`}
            >
              Alles anzeigen
        ...
        {loading && <p className="text-sm text-slate-200/80">Lade Cluster-Inhalte...</p>}
                ...
                {filtered.map((item) => (
                  ...
                  >
                    <div className="mb-3 flex items-start justify-between">
                      <span
                        className={`rounded-full border px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wide ${typeStyles[item.type]}`}
                      >
                        {item.type}
                      </span>
                      <span className="text-xs text-slate-400">{item.segmentName}</span>
                    </div>
                    <h3 className="text-base font-semibold text-white group-hover:text-nexo-aqua">
                      {item.name}
                    </h3>
              ...
              <div className="flex gap-4 overflow-x-auto pb-2">
                {segmentsToRender.map((segment) => {
                  ...
                  return (
                    <div
                      key={segment.slug}
                      className="min-w-[240px] flex-1 rounded-2xl border border-white/10 bg-slate-900/60 p-4 shadow-sm"
                    >
                      ...
                      <div className="space-y-3">
                        {items.map((item) => (
                          ...
                          >
                            <div className="mb-2 flex items-center justify-between text-xs">
                              <span
                                className={`rounded-full border px-2 py-0.5 font-semibold uppercase tracking-wide ${typeStyles[item.type]}`}
                              >
                                {item.type}
                              </span>
                        ...
                        {items.length === 0 && (
                          <p className="text-xs text-slate-400">Keine Inhalte in diesem Segment.</p>
                        )}
                      </div>
                    </div>
                  );
