...
import type { Cluster, MacroCluster, Segment, SegmentContent, SegmentContentType } from "@/types/grid";
...
export default function SegmentPage({ params }: { params: Promise<Params> }) {
  ...
  useEffect(() => {
    fetchGrid()
      .then((response) => {
        const foundMacro = response.data?.macroClusters?.find((macro) => macro.slug === clusterSlug) ?? null;
  ...
  const labelMap = useMemo(
    () =>
      contentGroups.reduce<Record<SegmentContentType, string>>(
        (acc, entry) => {
  ...
  const toggleType = (type: SegmentContentType) => setOpenType((prev) => (prev === type ? null : type));
  ...
  return (
    <>
      <BackButton href={`/grid/${macroCluster?.slug ?? ""}/${subcluster?.slug ?? ""}`} label="Zurück zum Sub-Cluster" />
      <header className="space-y-1">
        <p className="text-sm font-semibold uppercase tracking-[0.2em] text-slate-200/70">Segment</p>
        <h1 className="text-3xl font-semibold sm:text-4xl">{segment?.name ?? "Lade Segment..."}</h1>
        <p className="text-base text-gray-300">Wähle einen Content-Typ für {segment?.name ?? "dieses Segment"}.</p>
      </header>
              ...
              >
                ...
                >
                  <CardContent className="grid grid-cols-1 gap-3 px-5 pb-5 sm:grid-cols-2">
                    {items.map((item) => (
                      ...
                      >
                        <CardHeader className="pb-2">
                          ...
                          <p className="text-xs uppercase tracking-wide text-slate-400">{labelMap[type]}</p>
                        </CardHeader>
                    ...
                    {items.length === 0 && <p className="text-sm text-slate-200/75">Keine Einträge verfügbar.</p>}
                  </CardContent>
                </div>
              </Card>
            );
