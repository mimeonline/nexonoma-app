    ...
    <div className="space-y-8 pb-20">
      ...
      <section ref={heroRef} className="relative bg-nexo-card rounded-2xl border border-nexo-border p-8 shadow-card overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between gap-6">
          ...
          <div className="w-full flex-1 basis-0 min-w-0 max-w-3xl space-y-4 text-left">
            {/* 1. BADGES + TAGS */}
            ...
            <div className="flex items-start gap-4 pt-2">
              <DynamicIcon name={icon} className="h-8 w-8 md:h-10 md:w-10 text-muted-foreground shrink-0 mt-1" />
              <h1 className="text-3xl md:text-4xl font-bold text-white tracking-tight leading-tight">{content.name}</h1>
            </div>

            {/* 3. SHORT DESCRIPTION */}
            <p className="text-lg text-nexo-muted font-light leading-relaxed">{content.shortDescription}</p>

            {/* 4. EXPLAIN / WHAT IS THIS VIEW */}
            <Popover>
              <PopoverTrigger asChild>
                <button
                  type="button"
                  className={`inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-sm font-medium
                    text-slate-300/80 border border-white/10 bg-white/5
                ...
                >
                  ...
                  <span>{t("catalog.detail.explain.trigger")}</span>
                </button>
              </PopoverTrigger>
              ...
              >
                <p className="text-sm font-semibold text-white mb-2">{t("catalog.detail.explain.title")}</p>
                <p className="text-sm leading-relaxed text-slate-200/80">{t("catalog.detail.explain.body")}</p>
              </PopoverContent>
            </Popover>

            {/* 5. OPTIONAL HERO QUOTE */}
