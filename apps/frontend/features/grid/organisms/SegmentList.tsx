      ...
      {segments.map(({ segment, subclusterSlug }) => (
        ...
        >
          ...
          <CardContent className="flex items-center justify-between pt-0 text-sm text-slate-200/85">
            ...
            <Link href={`/grid/${clusterSlug}/${subclusterSlug}/${segment.slug}`} className="font-semibold text-[#4FF4E0] hover:underline">
              Öffnen
            </Link>
          </CardContent>
        </Card>
      ))}
      {segments.length === 0 && <p className="text-sm text-slate-200/75">Keine Segmente vorhanden.</p>}
    </div>
  );
}
